...
CodeWord& CodeWord::operator=(const CodeWord& cw)
{
    ...
    this->m_colors[0] = cw.m_colors[0];
    this->m_colors[1] = cw.m_colors[1];
    this->m_colors[2] = cw.m_colors[2];
...
CBModel::CBModel(unsigned int num_frame_train, ColorType color_typet, float e1/*=7.5*/,
                 float e2/*=15*/,
...
int CBModel::process(uchar* pColorImg, int width, int height, uchar* pUpdateMap, uchar* pFGMask,
                     uchar* pColorBGImg)
{
    if (!m_cb)
    ...
    if (m_curr_frame < m_num_train) {
        ...
        if (m_shadow_remove || pColorBGImg) {
    ...
    for (y = 0; y < m_rows; y++) {
        ...
        for (x = 0; x < m_cols; x++, p += 3, cb++) {
            ...
            for (i = 0; i < m_depth; i++) {
                ...
                if (abs(cw[i].m_colors[0] - p0) <= m_e1 && abs(cw[i].m_colors[1] - p1) <= m_e1 &&
                        abs(cw[i].m_colors[2] - p2) <= m_e1) {
        ...
        for (x = 0; x < m_cols; x++, p += 3, pp++, ppp++, cb++) {
            ...
            if (!isFound) { // foreground detected. Then find match from cache.
                ...
                for (i = 0; i < m_depth; i++) {
                    ...
                    if (abs(ca[i].m_colors[0] - p0) <= m_e1 && abs(ca[i].m_colors[1] - p1) <= m_e1 &&
                            abs(ca[i].m_colors[2] - p2) <= m_e1) {
...
void medianFilterBinary(uchar* mask, int width, int height, int win_sz)
{
    ...
    for (int y = offset; y < height - offset; y++) {
        ...
        for (int x = offset; x < width - offset; x++, p++, sum_ptr++) {
            int* p0 = /*sum_ptr - offset_sum_step - offset;*/           sum + (y - offset) * sum_step + x -
                                                                        offset;
            int* p1 = /*sum_ptr - offset_sum_step + offset + 1; */      sum + (y - offset) * sum_step + x +
                                                                        offset + 1;
            int* p2 = /*sum_ptr + offset_sum_step + sum_step - offset;*/    sum +
                                                                            (y + offset + 1) * sum_step + x - offset;
            int* p3 = /*sum_ptr + offset_sum_step + offset + 1; */      sum + (y + offset + 1) * sum_step + x +
                                                                        offset + 1;
